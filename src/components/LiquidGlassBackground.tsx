...
const LiquidGlassBackground: React.FC<LiquidGlassBackgroundProps> = ({
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none select-none z-0 ${className}`}>
      ...
      <div className="absolute -top-[20%] -left-[10%] w-[65%] h-[65%] liquid-orb-1 opacity-80" />

      {/* 3. Rose + deep blue orb - bottom-right */}
      <div className="absolute -bottom-[20%] -right-[10%] w-[70%] h-[70%] liquid-orb-2 opacity-75" />

      {/* 4. Rose accent orb - right mid */}
      <div className="absolute top-[20%] right-[5%] w-[55%] h-[55%] liquid-orb-rose opacity-55" />

      {/* 5. Secondary cyan orb - hero only, center-left */}
      {variant === 'hero' && (
        <div className="absolute top-[15%] left-[10%] w-[50%] h-[50%] liquid-orb-3 opacity-50" />
      )}

      {/* 6. CTA/footer small teal accent */}
      {variant !== 'hero' && (
        <div className="absolute top-[30%] left-[20%] w-[40%] h-[40%] liquid-orb-4 opacity-45" />
      )}
